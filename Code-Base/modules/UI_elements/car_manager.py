...
def show_car_manager(ui):
    ...
    row_count = 2
    for car in ui.cars:
        edit_button = tk.Button(ui.master, text="🖉", command=lambda carID=car["CarId"]: show_car_form(
            ui, editing=True, carID=carID))
...
def show_car_form(ui, editing=False, carID=None):
    ...
    if editing:
        title_label = ttk.Label(
            ui.master, text="Edit Car Information", font=("Arial", 16))
        ...
        submit_button = ttk.Button(
            ui.master, text="Submit", command=lambda carID=car["CarId"]: edit_car(ui, carID))
        submit_button.grid(row=8, columnspan=2, padx=BUTTON_X, pady=BUTTON_Y)

    else:
        title_label = ttk.Label(
            ui.master, text="Add a New Car", font=("Arial", 16))
        title_label.grid(row=0, column=0, columnspan=2, pady=TITLE_Y)

        submit_button = ttk.Button(
            ui.master, text="Submit", command=lambda: add_new_car(ui))
        submit_button.grid(row=8, columnspan=2, padx=BUTTON_X, pady=BUTTON_Y)
    ...
    year_entry = ttk.Entry(ui.master, textvariable=ui.year_var,
...
def add_new_car(ui):
    ...
    for service in op_list:
        user_response = messagebox.askyesno(
            "Add Service", f"{service["Question"]}?")
        if user_response:
            ui.db.add_recurring_services(
                service["Name"], car_id, service["DueMileage"], service["IntervalValue"], service["IntervalUnit"], None)
...
def remove_car(ui, carID):
    ...
    confirmed = messagebox.askyesno(
        "Delete Car", f"Are you sure you want to delete {car["Year"]} {car["Make"]} {car["Model"]}")
    if confirmed:
        mile_count = ui.db.get_mileage_by_ID(carID)
        if mile_count and len(mile_count) > 3:
            messagebox.showinfo(
                "Double Check", "Please go to the terminal to confirm the removal")
            double = input(
                f'CAUTION: You are about to delete a car with {len(mile_count)} entries, type "Yes" if you meant to do this: \n')
            if double == "Yes":
                safe_to_delete = True
            else:
                print(
                    "Operation aborted, if you believe this is a mistake, make sure you type Y-e-s")
        else:
            safe_to_delete = True
...
def pick_color(ui):
    color_code = colorchooser.askcolor(
        title="Choose color", initialcolor=ui.color_var.get())[1]
    if color_code != None:

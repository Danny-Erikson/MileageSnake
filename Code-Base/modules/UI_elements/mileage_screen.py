...
def show_mileage_screen(ui):
    ...
    # * Fuel Area
    mpg_check = tk.Checkbutton(
        ui.master, text="MPG Entry", variable=ui.mpg_var, command=lambda: mpg_toggle(ui))
    mpg_check.grid(row=4, columnspan=2, padx=BUTTON_X, pady=BUTTON_Y)

    ui.total_label = tk.Label(ui.master, text="Total Paid:")
    ui.total_label.grid(row=5, column=0, sticky="e",
                        padx=ENTRY_X, pady=ENTRY_Y)

    ui.total_entry = ttk.Entry(ui.master, textvariable=ui.total_paid_var,
                               validate="key", validatecommand=(ui.float_vcmd, "%P"), state="disabled")
    ui.total_entry.grid(row=5, column=1, sticky="w",
                        padx=ENTRY_X, pady=ENTRY_Y)

    ui.gallons_label = tk.Label(ui.master, text="Gallons Bought:")
    ui.gallons_label.grid(row=6, column=0, sticky="e",
                          padx=ENTRY_X, pady=ENTRY_Y)

    ui.gallons_entry = ttk.Entry(ui.master, textvariable=ui.gallons_bought_var,
                                 validate="key", validatecommand=(ui.float_vcmd, "%P"), state="disabled")
    ui.gallons_entry.grid(row=6, column=1, sticky="w",
                          padx=ENTRY_X, pady=ENTRY_Y)

    ui.full_up_check = tk.Checkbutton(
        ui.master, text="Filled to Full", variable=ui.full_up_var, state="disabled")
    ui.full_up_check.grid(row=7, columnspan=2, padx=BUTTON_X, pady=BUTTON_Y)

    # * Submit Area
    mile_submit = ttk.Button(
        ui.master, text="Enter Mileage", command=lambda: add_mileage(ui))

...
def show_recur_service_manager(ui):
    ...
    title_label = tk.Label(
        ui.master, text="Recurring Services", font=("Arial", 16))
    ...
    ui.car_combo.grid(row=1, column=0, columnspan=5,
    ...
    tk.Label(ui.service_table, text="Service").grid(
        row=0, column=1, sticky="ew")
    tk.Label(ui.service_table, text="Service Interval (Miles)").grid(
        row=0, column=2, sticky="ew")
    tk.Label(ui.service_table, text="Service Interval (Every)").grid(
...
def on_car_change(ui):
    ...
    for service in services:
        edit_button = tk.Button(
            ui.service_table,
            text="🖉",
            command=lambda serviceID=service["ServiceId"]: show_service_form(
                ui, editing=True, serviceID=serviceID)
        )
        ...
        service_in_miles = tk.Label(
            ui.service_table, text=f"{service["DueMileage"]:,}" if service["DueMileage"] is not None else "")
        service_in_miles.grid(row=row_count, column=2, sticky="ew")

        service_in_days = tk.Label(
            ui.service_table, text=f"{day_formatter(service["IntervalValue"], service["IntervalUnit"])}" if service["IntervalValue"] is not None else "")
        service_in_days.grid(row=row_count, column=3, sticky="ew")

        remove_service_button = tk.Button(
            ui.service_table,
            text="Remove Service",
            command=lambda serviceID=service["ServiceId"]: remove_service(
                ui, serviceID=serviceID)
        )
...
def show_service_form(ui, serviceID=None, editing=False):
    ...
    if editing:
        ...
        submit_button = ttk.Button(
            ui.master, text="Submit", command=lambda serviceID=service["ServiceId"]: edit_service(ui, serviceID))
    ...
    car_label = ttk.Label(
        ui.master, text=f"Service for: {ui.cars[ui.car_index]["Year"]} {ui.cars[ui.car_index]["Make"]} {ui.cars[ui.car_index]["Model"]}", font=(14))
    ...
    mileage_entry.grid(row=3, column=1, sticky="we",
                       padx=ENTRY_X, pady=ENTRY_Y)
    ...
    notes_instr_label = ttk.Label(
        ui.master, text='AutoNote is automatically added to the service entry when entered\nI.E. if you used Valvoline oil, you could put "Used Valvoline oil')
    notes_instr_label.grid(row=6, column=0, columnspan=3,
                           padx=ENTRY_X, pady=ENTRY_Y)

    back_button = ttk.Button(ui.master, text="Go Back",
                             command=lambda: show_recur_service_manager(ui))
...
def edit_service(ui, serviceID):
    ...
    ui.db.update_recurring_service(
        *sanitized_inputs, ui.note_var.get(), serviceID)
    show_recur_service_manager(ui)

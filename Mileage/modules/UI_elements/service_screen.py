...
def show_service_screen(ui):
    ...
    ui.car_combo.bind("<<ComboboxSelected>>",
                      lambda e: build_service_editor(ui))
    ...
    mileage_entry = ttk.Entry(ui.master, textvariable=ui.mileage_var,
    ...
    mileage_date_label.grid(row=3, column=0, sticky="e",
    ...
    reoccurring_radio = ttk.Radiobutton(ui.master, text="Reoccurring Service",
                                        variable=ui.service_mode_var, value="reoccurring", command=lambda: build_service_editor(ui))
    reoccurring_radio.grid(row=4, column=0, sticky="e",
                           padx=ENTRY_X, pady=ENTRY_Y)

    general_radio = ttk.Radiobutton(ui.master, text="General Service", variable=ui.service_mode_var,
                                    value="general", command=lambda: build_service_editor(ui))
...
def build_general_service(ui):
    ...
    description_label.grid(row=1, column=1, sticky="e",
                           padx=ENTRY_X, pady=ENTRY_Y)

    ui.description_box = tk.Text(ui.service_editor, width=40, height=10)
    ui.description_box.grid(row=1, column=2, sticky="w",
                            padx=ENTRY_X, pady=ENTRY_Y)

    submit_button = ttk.Button(
        ui.service_editor, text="Submit", command=lambda: add_one_service(ui))
...
def build_reoccurring_service(ui):
    ...
    for s in services:
        ...
        tk.Checkbutton(ui.service_editor, text=s["Name"], variable=ui.selected[service_id],
                       command=lambda sid=service_id: toggle_note(sid)).grid(row=row_count, column=1, sticky="w")
        tk.Label(ui.service_editor, text="Notes: ").grid(
            row=row_count, column=1, sticky="e")
        ui.note_entries[service_id] = tk.Entry(
            ui.service_editor, textvariable=ui.notes[service_id], state="disabled")
        ui.note_entries[service_id].grid(
            row=row_count, column=2, ipadx=75, sticky="w")

        row_count += 1

    tk.Button(ui.service_editor, text="Submit", command=lambda: add_reoccurring_services(
        ui)).grid(row=row_count + 1, columnspan=4)
...
def add_one_service(ui):
    ...
    mileage_check = ui.db.mileage_match(
        ui.cars[ui.car_index]["CarId"], ui.mileage_var.get(), ui.date_var.get())
    if mileage_check != None:
        mileage_id = mileage_check["MileageId"]
    else:
        mileage_id = ui.db.add_mileage(
            ui.cars[ui.car_index]["CarId"], ui.mileage_var.get(), ui.date_var.get())

    ui.db.add_service(ui.name_var.get(
    ), ui.cars[ui.car_index]["CarId"], None, mileage_id, ui.description_box.get("1.0", "end"))
...
def add_reoccurring_services(ui):
    ...
    mileage_check = ui.db.mileage_match(
        car_id, ui.mileage_var.get(), ui.date_var.get())
    if mileage_check != None:
        mileage_id = mileage_check["MileageId"]
    else:
        mileage_id = ui.db.add_mileage(
    ...
    for ser_id in selected_ids:
        ...
        if ui.notes[ser_id].get() != "":
            if ui.notes[ser_id].get() != ser["AutoNote"]:
                update = messagebox.askquestion(
                    "Update AutoNote", f'We noticed you updated the note for "{ser["Name"]}"\nDid you want to update it from\n"{ser["AutoNote"]}"\nto\n"{ui.notes[ser_id].get()}"')
                if update:
                    ui.db.update_auto_note_by_id(
                        ui.notes[ser_id].get(), ser_id)
                    ui.db.add_service(
                        ser["Name"], car_id, ser_id, mileage_id, ui.notes[ser_id].get())
                else:
                    ui.db.add_service(ser["Name"], car_id,
                                      ser_id, mileage_id, ser["AutoNote"])
            else:
                ui.db.add_service(ser["Name"], car_id,
                                  ser_id, mileage_id, ser["AutoNote"])
        else:

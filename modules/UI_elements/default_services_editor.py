...
def show_default_services_editor(ui):
    ...
    #* Top of screen
    title_label = tk.Label(ui.master, text="Default Services", font=("Arial", 16))
    title_label.grid(row=0, column=0, columnspan=5, pady=TITLE_Y)
    
    top_text = tk.Label(ui.master, text="Below are the services that will be auto generated when a new car is added.")
    ...
    for service in auto_service:
        edit_button = tk.Button(
            ui.master,
            text="🖉",
            command=lambda templateId=service["TemplateId"]: show_service_form(ui, editing=True, templateId=templateId)
        )
        ...
        service_in_miles = tk.Label(ui.master, text=f"{service["DueMileage"]:,}" if service["DueMileage"] is not None else "")
        service_in_miles.grid(row=row_count, column=2, sticky="ew")
        
        service_in_days = tk.Label(ui.master, text=f"{day_formatter(service["IntervalValue"], service["IntervalUnit"])}" if service["IntervalValue"] is not None else "")
        service_in_days.grid(row=row_count, column=3, sticky="ew")
        
        remove_service_button = tk.Button(
            ui.master,
            text="Remove Service",
            command=lambda templateId=service["TemplateId"]: remove_service(ui, templateId=templateId)
        )
    ...
    #* Bottom of screen
    add_new = tk.Button(ui.master, text="Add New Service", command=lambda: show_service_form(ui))
    add_new.grid(row=row_count, column=0, columnspan=5, padx=BUTTON_X, pady=BUTTON_Y)
...
def show_service_form(ui, templateId=None, editing=False):
    ...
    if editing:
        ...
        ui.optional_var.set(value= True if service["IsOptional"] == 1 else False)
        ui.question_var.set(value= service["Question"] or "")
        
        submit_button = ttk.Button(ui.master, text="Submit", command=lambda templateId=service["TemplateId"]:edit_service(ui, templateId))
    ...
    mileage_entry.grid(row=2, column=1, sticky="we", padx=ENTRY_X, pady=ENTRY_Y)
    ...
    ask_checkbox = tk.Checkbutton(ui.master, text="Ask to Create (for services that apply to some car)",variable=ui.optional_var, command=lambda: question_toggle(ui))
    ask_checkbox.grid(row=4, columnspan=3, padx=ENTRY_X, pady=ENTRY_Y)
    
    question_label = ttk.Label(ui.master, text="Question:")
    question_label.grid(row=5, column=0, sticky="e", padx=ENTRY_X, pady=ENTRY_Y)
    ui.question_entry = ttk.Entry(ui.master, textvariable=ui.question_var)
    ui.question_entry.grid(row=5, column=1, columnspan=2, sticky="we", padx=ENTRY_X, pady=ENTRY_Y)
    
    question_toggle(ui)
    
    question_instr = question_label = ttk.Label(ui.master, text="When a new car is added, a messagebox with that question will pop up.\nMake sure the question can be answered with Yes or No")
    question_instr.grid(row=6, columnspan=3)
    
    back_button = ttk.Button(ui.master, text="Go Back", command=lambda: show_default_services_editor(ui))
...
def validate_inputs(ui):
    ...
    if ui.optional_var.get() == True and ui.question_var.get() == "":
        messagebox.showerror("Input Error", "Question cannot be blank for a optional service")
        return False
...
def add_service(ui):
    ...
    if ui.time_value_var.get() == "":
        ...
        safe_inputs = (*good_inputs, ui.time_value_var.get(), ui.time_unit_var.get())
    
    sanitized_inputs = (*safe_inputs, 1 if ui.optional_var.get() else 0)
    
    ui.db.add_services_template(*sanitized_inputs, ui.question_var.get() if ui.optional_var.get() else "")
...
def edit_service(ui, templateId):
    ...
    ui.db.update_services_template(*sanitized_inputs, ui.question_var.get() if ui.optional_var.get() else "", templateId)
